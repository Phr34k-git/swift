...
public sealed class ColoredTextLineViewModel
{
    public ColoredTextLineViewModel(IReadOnlyList<ColoredTextSegmentViewModel> segments)
    {
        Segments = segments;
    }

    public IReadOnlyList<ColoredTextSegmentViewModel> Segments { get; }
}

public sealed class ColoredTextSegmentViewModel
{
    public ColoredTextSegmentViewModel(string text, IBrush brush)
    {
        Text = text;
        Brush = brush;
    }
...
internal static class ColoredEnchantText
{
    private static readonly IReadOnlyList<string> KnownNames = EnchantColors.GetKnownEnchantNames()
    ...
    private static IReadOnlyList<ColoredTextSegmentViewModel> BuildSegments(string text)
    {
        ...
        while (index < text.Length)
        {
            ...
            result.Add(new ColoredTextSegmentViewModel(text.Substring(index, match.Length), EnchantColors.GetEnchantBrush(match)));
            index += match.Length;
            plainStart = index;
        }
