...
public sealed class Tracking3FishingTracker : IFishingTracker
{
    ...
    public Tracking3FishingTracker()
    {
        _memory = new RobloxMemory(OffsetsSourceProvider.Current);
        _context = new FishingRuntimeContext(_memory);
        Status = new FishingTrackerStatus(false, "OFF", "Tracking 3 ready.", null, false);
    }

    public FishingTrackerMode Mode => FishingTrackerMode.Tracking3;
    ...
    public void Stop()
    {
        ...
        _timer?.Dispose();
        _timer = null;
        StopCycle("OFF", "Tracking 3 stopped.");
    }
    ...
    private void StartCycle()
    {
        ...
        SetStatus("CASTING", "Casting with Tracking 3.");
    }
    ...
    private void UpdateFishingPhase()
    {
        ...
        if (metrics is not null)
        {
            ...
            SetStatus("FISHING", $"{decision.Mode} err {decision.Error:0.000}, ctrl {decision.Control:0.000}.");
            return;
        }
        ...
        {
            SetStatus("FISHING", "Reel context lost; waiting for grace period.");
        }
    }
    ...
    {
        return GetWorldStatusText("4_cycle").Contains("night", StringComparison.OrdinalIgnoreCase);
    }
    ...
    private bool IsWorldStatusMatchVisible(string statusName, string needle)
    {
        ...
        return GetWorldStatusText(statusName).Contains(needle, StringComparison.OrdinalIgnoreCase);
    }

    private bool GetWorldStatusVisible(string statusName)
    {
        var status = ResolveWorldStatus(statusName);
        return status != 0 && _memory.IsVisible(status, "FrameVisible");
    }
